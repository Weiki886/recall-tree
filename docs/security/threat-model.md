...
| 记忆向量（`memory_embedding`） | 中高 | 向量编码了记忆文本语义，学界已有嵌入反演（embedding inversion）研究可从向量部分重建原文，因此不能视为脱敏数据 |
...
| T19 | 向量被反演重建出记忆原文 | Information Disclosure | 向量按 `owner_id` 隔离且附带 `owner_id` 查询条件，与结构化数据共享同一鉴权边界；`memory_embedding` 表不含原文，内容不可通过向量直接查询，需通过 `memory_version` 加 `owner_id` 条件获取；首版无额外差分隐私保护（见第 4 节非目标） |
...
需要强调的是：不做差分隐私**不等于**可以放松向量的访问控制。`memory_embedding` 与记忆内容同属高敏感数据，同样受 Owner 隔离与硬删除约束（T19、验收项 2）。
