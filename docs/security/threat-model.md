...
以下每项都要有对应自动化测试，作为安全声明的证据：

1. 两个 Owner 的记忆互不可见（跨 Owner 请求返回 404）；
2. 硬删除后版本、来源、向量三张表均无残留；
3. 畸形模型输出不产生任何 `CURRENT` 版本；
4. 提示注入样例无法删除或篡改已有记忆；
5. 日志与错误响应中不出现 API Key 或数据库凭据；
6. 用户修正过的记忆不被后续模型提取自动覆盖。

这六条对应 T2、T8、T13、T12、T11 与 T12，在 Issue #9 与 Issue #6 实现时必须落为测试。
